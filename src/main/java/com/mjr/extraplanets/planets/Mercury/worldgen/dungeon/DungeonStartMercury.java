...
public class DungeonStartMercury extends EntranceCraterMercury {
	...
	public void buildComponent(StructureComponent componentIn, List<StructureComponent> listIn, Random rand) {
		...
		while (!validAttempt && attempts < maxAttempts) {
			...
			if (attachedComponents.size() >= 3 && attachedComponents.get(attachedComponents.size() - 1) instanceof RoomTreasureMercury && attachedComponents.get(attachedComponents.size() - 3) instanceof RoomBossMercury) {
				validAttempt = true;
			}
			attempts++;
		}
